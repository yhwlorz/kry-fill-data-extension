...
export const simulateInput = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      if (setInputValue(divEl as HTMLInputElement)) {
        resolve(null);
        return;
      }
    }
      ...
      for (let mutation of mutationsList) {
        if (mutation.type === "childList") {
          ...
          if (inputEl && setInputValue(inputEl)) {
            clearTimeout(timeoutId);
            observer.disconnect();
            //当异步操作成功时，应该调用resolve函数，并把结果作为参数传递出去。resolve(null);表示输入操作成功，但没有返回值
            resolve(null);
            return;
          }
        }
    ...
    //   console.log("所有dom变化。mutationsList:", mutationsList);
    //   // 遍历所有的mutations
    //   for (let mutation of mutationsList) {
    //     console.log("单个dom变化类型mutation.type:", mutation.type);
    //     // 如果div元素的子元素发生了变化
    //     if (mutation.type === "childList") {
    //       // 获取div元素下的input元素
    //       const inputEl = divEl.querySelector("input");
    ...
    // console.log("已进入simulateInput方法");
    // setTimeout(() => {
    //   // 点击div
    //   divEl.click();
    // }, 100);
